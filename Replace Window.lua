...
imgui.OnInitialize(function()
	...
	imgui.GetIO().Fonts:AddFontFromFileTTF(getWorkingDirectory() .. '/ReplaceWindow/EagleSans-Regular.ttf', 20, nil, glyph_ranges)

	-->> Other Fonts
	for k, v in ipairs({15, 18, 25, 30}) do
		fonts[v] = imgui.GetIO().Fonts:AddFontFromFileTTF(getWorkingDirectory() .. '/ReplaceWindow/EagleSans-Regular.ttf', v, nil, glyph_ranges)
	end

	-->> Logo
	logo = imgui.CreateTextureFromFile(getWorkingDirectory() .. '/ReplaceWindow/Logo.png')
end)
...
local windowFrame = imgui.OnFrame(
	...
	function(player)
		...
		imgui.Begin(thisScript().name, window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysUseWindowPadding)
			...
			imgui.BeginChild('right', imgui.ImVec2(-1, -1), true)
				if menu == 1 then
					...
					imgui.NewLine()
						...
						imgui.FText(u8('{TextDisabled}[Ответ]: {Text}При покупке подписки назови промокод "{ButtonActive}{Text}", чтобы получить {ButtonActive}+1 бесплатный месяц{Text} к'))
				...
				elseif menu == 2 then
					...
					imgui.BeginColorChild('log', imgui.ImVec2(-1, -1), false)
						if date_select and jsonLog[date_select][1] then
							if logMenu == 0 then
								...
								for k, v in ipairs(jsonLog[date_select][1]) do
									...
									local item = fullString:find('%(%d+ шт%.%)$') and fullString:gsub('%(%d+ шт%.%)$', '') or fullString	
									if not arr[item] then arr[item] = {['buy'] = 0, ['sell'] = 0} end
									arr[item][v:find('продал') and 'buy' or 'sell'] = arr[item][v:find('продал') and 'buy' or 'sell'] + count
								end

								for k, v in pairs(arr) do
									imgui.SetCursorPosX(2)
									imgui.FText(u8(('{Text}[{ButtonActive}%s{Text}] | Купили у меня: {ButtonActive}%s{Text} | Продали мне: {ButtonActive}%s'):format(k:gsub(' $', ''), v.sell, v.buy)))
								end
							end
						else
							imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() - imgui.CalcTextSize(u8('Здесь пока ничего нету :(')).x) / 2, (imgui.GetWindowHeight() - 20) / 2))
				...
				elseif menu == 6 then
					...
					imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() * 0.5 - 365) / 2 + 5, (imgui.GetWindowHeight() - 355) / 2))
					imgui.BeginColorChild('settingsWindow', imgui.ImVec2(365, 355), false)
						imgui.StripChild()
						imgui.BeginColorChild('settingsWindowUnder', imgui.ImVec2(-1, -1), false)
							...
							imgui.BeginGroup()
								...
								if imgui.ActiveButton(u8(marketBool.always[0] and 'Да' or 'Нет'), imgui.ImVec2(170)) then
					...
					imgui.BeginGroup()
						...
						imgui.BeginGroup()
							imgui.BeginColorChild('settingsNotf', imgui.ImVec2(365, 295), false)
								imgui.StripChild()
...
local updateFrame = imgui.OnFrame(
	function() return (infoGithub.version and infoGithub.version.v ~= thisScript().version_num) and not isPauseMenuActive() and not sampIsScoreboardOpen() end,
	function(player)
	  imgui.SetNextWindowPos(imgui.ImVec2(select(1, getScreenResolution()) / 2, select(2, getScreenResolution()) / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		...
		imgui.Begin('update', update, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
			...
			imgui.BeginChild('update', imgui.ImVec2(-1, -40), false)
				for k, v in ipairs(infoGithub.update[#infoGithub.update].text) do
					imgui.SameLine(10); imgui.FText(('{ButtonActive}%s) {Text}%s'):format(k, v), 20)
				end
			imgui.EndChild()

			if imgui.Button(u8('Отмена'), imgui.ImVec2(150, -1)) then infoGithub.version.v = thisScript().version_num end
			imgui.SameLine(imgui.GetWindowWidth() - 155)
			if imgui.ActiveButton(u8('Установить'), imgui.ImVec2(150, -1)) then
				downloadUrlToFile(infoGithub.version.url, getWorkingDirectory() .. '/Update.txt', function(id, status, p1, p2)
					if status == dlstatus.STATUS_ENDDOWNLOADDATA then
						sms('Загрузка обновления завершена.')
						lua_thread.create(function() wait(500) thisScript():reload() end)
						-- infoGithub.version.v = thisScript().version_num
					end
				end)
			end
	...
	function(player)
		...
		imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(marketColor.window[0], marketColor.window[1], marketColor.window[2], marketAlpha[0]))
			imgui.Begin('market', market, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
				...
				for k, v in ipairs(marketShop) do
					imgui.Text(u8(v))
				end
			imgui.End()
...
function samp.onServerMessage(color, text)
	...
	for k, v in ipairs(hookMarket) do
		if string.find(text, v['text']) and v['color'] == color then
			-- local args = splitArguments({text:match(v['text'])}, v['key'])
	...
	if text:find('^%s*%(%( Через 30 секунд вы сможете сразу отправиться в больницу или подождать врачей %)%)%s*$') then
		...
		if notifications[5][3][0] then
			sendTelegram('Ваш персонаж умер!')
		end
	end
...
function splitArguments(array, key)
	-- return {
	-- 	['name'] = (key == 2 and array[1] or array[2]),
	-- 	['item'] = (key == 2 and array[2] or array[1]),
	-- 	['ViceCity'] = array[3],
...
-->> Other Function
function getInfo()
	local path_download = os.getenv('TEMP') .. '/ReplaceWindow.json'
	local url = 'https://raw.githubusercontent.com/Xkelling/Replace-Window/main/test'
	downloadUrlToFile(url, path_download, function(id, status, p1, p2)
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			local file = io.open(path_download, 'r')
			if file then
				infoGithub = decodeJson(file:read('*a'))
				file:close()
			end
		end
	end)
