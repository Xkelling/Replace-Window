		...
		imgui.Begin(thisScript().name, window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysUseWindowPadding)
			...
			imgui.BeginChild('right', imgui.ImVec2(-1, -1), true, imgui.WindowFlags.NoScrollbar)
				...
				elseif menu == 6 then
					...
					imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() * 0.5 - 365) / 2 + 5, (imgui.GetWindowHeight() - 415 + 30) / 2))
					imgui.BeginColorChild('settingsWindow', imgui.ImVec2(365, 415), false)
						imgui.StripChild()
					...
					imgui.BeginGroup()
						...
						imgui.BeginGroup()
							imgui.BeginColorChild('settingsNotf', imgui.ImVec2(365, 355), false)
								imgui.StripChild()
...
function samp.onServerMessage(color, text)
	...
	if color == -2686721 and text:find('.- .-: .+') and notifications[8][3][0] and not text:find('Сообщение до редакции:') then
		sendTelegram(text)
	end

	if text:find('^%s*%[Подсказка%] {FFFFFF}Вы успешно арендовали лавку для продажи/покупки товара!%s*$') and notifications[9][3][0] then
		sendTelegram('Вы успешно арендовали лавку!')
	end
end
