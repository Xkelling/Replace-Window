...
if not imgui_check or not samp_check or not effil_check or not requests_check or not monet_check then
	function main()
		...
		for k, v in pairs(libs) do
			if not v then sampAddChatMessage('« Replace Window » {FFFFFF}У Вас отсутствует библиотека {7172ee}' .. k .. '{FFFFFF}. Сейчас скрипт попробует её {7172ee}установить{FFFFFF}!', 0x7172ee); table.insert(libs_no_found, k) end
		end
		sampShowDialog(18364, '{7172ee}Replace Window', string.format('{FFFFFF}В Вашей сборке {7172ee}нету необходимых библиотек{FFFFFF} для работы скрипта.\nБез них он {7172ee}не будет{FFFFFF} работать!\n\nБиблиотеки, которые Вам нужны:\n{FFFFFF}- {7172ee}%s\n\n{FFFFFF}Сейчас скрипт попробует их {7172ee}установить{FFFFFF}!', table.concat(libs_no_found, '\n{FFFFFF}- {FF5545}')), 'Принять', '', 0)
		thisScript():unload()
	end
	return
end
...
function main()
	...
	sms('Тестовая string ' .. thisScript().version)
	sms('Активация: /repw')
	getInfo()
...
imgui.OnInitialize(function()
	...
	-->> Logo
	logo = imgui.CreateTextureFromFile(getWorkingDirectory() .. '/ReplaceWindow/ReplaceWindow.png')
end)
...
-->> Other Function
function getInfo()
	asyncHttpRequest('GET', 'https://raw.githubusercontent.com/Xkelling/Replace-Window/main/test', nil, function(response) infoGithub = decodeJson(response.text) end)
